...
// Material itself, such as a Card, Dialog, Drawer, or Scaffold.
  testWidgets('Login with accepted email and password',
    ...
    await tester.pump();
    expect(find.byKey(const Key('AlertDialogLoginFlow')), findsOneWidget);
    ...
    await tester.enterText(
        find.byKey(const Key('PasswordFormLoginTextFormField')), 'T3STU1D1');
  ...
  testWidgets('Go to register page', (WidgetTester tester) async {
    ...
    await tester.tap(find.byType(ElevatedButton));
    await tester.pump();
    expect(find.byKey(const Key('RegisterFormLogin')), findsOneWidget);
    ...
    await tester.pump();
    await tester.enterText(
        find.byKey(const Key('DisplayNameNameRegisterFormLogin')), 'Dom Builder');
    await tester.enterText(
        find.byKey(const Key('PasswordRegisterFormLogin')), 'T3STU1D');
    await tester.tap(find.byType(ElevatedButton));
    await tester.pump();
    expect(find.byKey(const Key('Selling')), findsOneWidget);
  });
